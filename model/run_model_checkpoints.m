...
if strcmp(params.solver,'odeRK')
    ...
    solver = @(tt) odeRK(odefun, tt, v0, params.solver_opts);
%     [~, Y] = odeRK(odefun, params.tt, v0, params.solver_opts);

elseif strcmp(params.solver, 'odeRKplus')
    disp('Using adaptive RK time stepping')

    % Add the appropriate output function to display solver progress
    params.solver_opts.OutputFcn = OutFcn;
    params.solver_opts.stats = stats;

%     [~, Y] = odeRKplus(odefun, params.tt, v0, params.solver_opts);
    solver = @(t) odeRKplus(odefun, tt, v0, params.solver_opts);
...
%     [~, Y] = odesolver(odefun, params.tt, v0, params.solver_opts);
    solver = @(t) odesolver(odefun, tt, v0, params.solver_opts);
